    ...
    <div class="card m-0">
      ...
      <div class="card-body">
        <table class="table">
          ...
          <tbody>
            <tr v-for="(box, i) in boxes" :key="box._id">
              <td class="text-center">{{ i + 1 }}</td>
              <td>{{ box.title }}</td>
              <td>{{ box.price }} <i class="fa-light fa-crown"></i></td>
              <td>
                <span
                  :class="
                    `badge ${
                      box.status ? 'badge badge-success' : 'badge badge-danger'
                    }`
                  "
                  >{{ box.status ? "Available" : "Unavailable" }}</span
                >
              </td>
              <td class="td-actions text-right">
                <router-link
                  :to="{ name: 'EditBox', params: { id: box._id } }"
                  class="btn btn-success btn-icon btn-sm btn-simple mr-2"
                >
                  <i class="fa-duotone fa-pen-to-square"></i>
                </router-link>
                <button
                  @click="destroy(box._id)"
  ...
  setup() {
    ...
    watchEffect(() => {
      if (store.getters.user.token) {
        api
          .get(`/admin/boxes?page=${page.value}`, {
        ...
        if (result.isConfirmed) {
          ...
          api
            ...
            .then(() => {
              ...
              boxes.value = boxes.value.filter((box) => box._id !== boxId);
            })
            .catch((err) => {
              console.log(err.response);
              store.dispatch("loading", false);
    ...
    return { boxes, destroy, pages, page, changePage };
  },
};
</script>
