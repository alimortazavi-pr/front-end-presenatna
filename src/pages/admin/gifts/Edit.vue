  ...
  <div class="container">
    <div class="card m-0">
      <div class="card-header bg-gradient-dark">
        <h4 class="text-white">Edit Gift</h4>
      </div>
      <div class="card-body">
        <form>
            ...
            <div class="col-md-4">
              ...
              >
                ...
                <option
                  ...
                  :key="i"
                  :value="occasion._id"
                  :selected="occasion._id === form.occasion"
  ...
  setup() {
    ...
    watchEffect(() => {
      if (store.getters.user.token) {
        api
          .get(`/admin/gifts/${route.params.id}`, {
            ...
            .catch((err) => {
              if (err.response.status === 403) {
                Swal.fire({
                  icon: "error",
                  title: "Error",
                  text: "The gift must have at least one image!",
                });
              } else {
                console.log(err.response);
              }
    ...
    function submit() {
      ...
      api
        .put(`/admin/gifts/${route.params.id}`, data, {
          headers: { token: store.getters.user.token },
        })
        .then(() => {
          store.dispatch("loading", false);
